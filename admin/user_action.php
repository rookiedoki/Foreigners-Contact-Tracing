...
if(isset($_POST["action"])){
  ...
  if($_POST["action"] == "user_fetch"){
    ...
    while ($row = mysqli_fetch_assoc($usersRecords)){
      ...
      {
        $status = '<label class="badge badge-success">Negative</label>';
      }

      if($row["status"] == "Inactive")
      {
        $status = '<label class="badge badge-danger">Positive</label>';
      }
      ...
      $data[] = array(
        ...
        "email"           =>  $row['email'],
        "status"          =>  $status,
        "update"          =>  '<button type="button" class="btn btn-warning update_user" id="'.$row['id'].'"><i class="fas fa-edit"></i></button>'
      );
    }
